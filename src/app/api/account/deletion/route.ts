...
export async function POST(req: NextRequest): Promise<NextResponse> {
  ...
  const upstream = await fetch(`${SAFE_LINK_V3_API_BASE_URL}/api/v1/account/deletion`, {
    method: "POST",
    headers: {
      ...(cookie ? { cookie } : {}),
  ...
  if (upstream.ok) {
    ...
    response.cookies.set("SAFE_LINK_SESSION", "", {
      path: "/", maxAge: 0, expires: expired, httpOnly: true,
      secure: process.env.NODE_ENV === "production", sameSite: "lax",
    });
    response.cookies.set("XSRF-TOKEN", "", {
      path: "/", maxAge: 0, expires: expired, httpOnly: false,
      secure: process.env.NODE_ENV === "production", sameSite: "lax",
    });
  }
  return response;
