...
- JSON 형식으로만 응답: {"tips": ["수칙1", "수칙2", "수칙3"]}`;
    ...
    try {
        ...
        return Response.json({ tips: parsed.tips.slice(0, 3) });
    } catch (e: any) {
        console.error("[AI-Tips] Gemini failed:", e.message);
        return Response.json({ tips: getFallbackTips() });
    }
}
...
function getFallbackTips(): string[] {
    return [
        "🏗️ 고소 작업 시 안전대 착용 여부를 반드시 확인하시고, 하부 통제 구역을 설정해 주시기 바랍니다.",
