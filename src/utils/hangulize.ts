...
// 히라가나/가타카나 → 한글 발음 직접 매핑 (국립국어원 외래어 표기법)
const japaneseMap: Record<string, string> = {
    ...
    "じゃ": "쟈", "じゅ": "쥬", "じょ": "죠", "びゃ": "뱌", "びゅ": "뷰", "びょ": "뵤",
    "ぴゃ": "퍄", "ぴゅ": "퓨", "ぴょ": "표",
    "キャ": "캬", "キュ": "큐", "キョ": "쿄", "シャ": "샤", "シュ": "슈", "ショ": "쇼",
    "チャ": "챠", "チュ": "츄", "チョ": "쵸", "ニャ": "냐", "ニュ": "뉴", "ニョ": "뇨",
    "ヒャ": "햐", "ヒュ": "휴", "ヒョ": "효", "ミャ": "먀", "ミュ": "뮤", "ミョ": "묘",
    "リャ": "랴", "リュ": "류", "リョ": "료", "ギャ": "갸", "ギュ": "규", "ギョ": "교",
    "ジャ": "쟈", "ジュ": "쥬", "ジョ": "죠", "ビャ": "뱌", "ビュ": "뷰", "ビョ": "뵤",
    "ピャ": "퍄", "ピュ": "퓨", "ピョ": "표",
    // 히라가나 단일
    "あ": "아", "い": "이", "う": "우", "え": "에", "お": "오",
    "か": "카", "き": "키", "く": "쿠", "け": "케", "こ": "코",
...
function addBatchimChar(str: string, batchim: "ㄴ" | "ㅅ"): string {
    if (!str) return str;
    const code = str.charCodeAt(str.length - 1);
    if (code >= 44032 && code <= 55203 && (code - 44032) % 28 === 0) {
        return str.slice(0, -1) + String.fromCharCode(code + BATCHIM_MAP[batchim]);
    }
    return str;
}
...
function hangulizeJapanese(text: string): string {
    ...
    while (i < text.length) {
        ...
        if (ch === "っ" || ch === "ッ") {
            if (parts.length > 0) {
                parts[parts.length - 1] = addBatchimChar(parts[parts.length - 1], "ㅅ");
            }
            i++;
            continue;
        }
        if (ch === "ん" || ch === "ン") {
            if (parts.length > 0) {
                parts[parts.length - 1] = addBatchimChar(parts[parts.length - 1], "ㄴ");
            } else {
                parts.push("ン");
            }
            i++;
        ...
        if (i + 1 < text.length) {
            ...
            if (japaneseMap[two] !== undefined) {
                parts.push(japaneseMap[two]);
                i += 2;
                continue;
            }
        }
        if (japaneseMap[ch] !== undefined) {
            parts.push(japaneseMap[ch]);
        } else if (ch === " " || ch === "\u3000") {
            parts.push(" ");
        } else {
