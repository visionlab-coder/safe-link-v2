...
> **⚠️ 방향 전환(2026-06-23)**: 모바일 상용앱은 범소프트웨어가 네이티브 제작(기존 자산 최대 재사용). 우리=PoC 유지+벤더 지원. 자체 네이티브/스토어 중단. 상세 `GOAL.md` 상단.

1. VH-002 — 벤더 확정사항 반영 + 이관 지원
   - Done: 범소프트웨어 회신(① 백엔드 유지범위 ② repo 인수 여부 ③ PoC 운영주체 ④ 일정·시크릿 전달 채널)을 받아 `docs/vendor-handoff` 보완 + 질의 응답.
   - Scope: 문서/지원. 합의된 API 변경만 코드 반영. 그 전까지 현장 PoC 안정 유지(운영/DB 변경 자제).

### (이전 모바일 자체제작 — PoC 한정 / 벤더 이관)
- 미완 항목 #2 FCM·#4 실기기 검증은 **PoC 한정**으로만 의미. 스토어용은 벤더 트랙.
- 옵션(딥링크/FCM/오프라인캐시 등) 자체 진행은 **중단** — 벤더가 네이티브로 재구현.
