...
- 크기: 4,275,450 bytes
- SHA-256: `767c1ab40e91f5c5ce60d14d40a333c0955dc66c79611ece5acd056a83ea774e`
- 동작: 앱 실행 시 `server.url`로 **배포 웹앱 전체**(`https://safe-link-v2.vercel.app`)를 first-party WebView로 로드. 관리자·근로자 모두 웹앱 로그인으로 전 기능 사용. 실행 시 마이크/카메라/알림 권한 요청.
...
- 사전조건: 로그인 상태.
- 단계:
  1. 비행기 모드 ON → "런타임 상태" Network=OFFLINE 확인.
  2. 비행기 모드 OFF → ONLINE 복귀 확인.
- 기대결과: 네트워크 상태 실시간 반영, 오프라인 시 호출 실패가 사용자에게 명확히 표시(크래시 없음).
- 기록: Android [ ] / iOS [ ] · 비고:
