# SAFE-LINK NFC 인증 API 스펙 — 하이정보 회신용 (검토본 v1.1)

> **발신**: 서원토건 미래전략TF 김무빈 차장  
> **수신**: 하이정보 담당자  
> **작성일**: 2026-05-20  
> **검토·수정일**: 2026-05-20  
> **버전**: v1.1 (SAFE-LINK 코드 기반 재검증 완료)  
...
| 항목 | v1.0 (초안) | v1.1 (수정) | 사유 |
|------|-------------|-------------|------|
| 토큰 길이 | 16자 hex | **24자 hex** | 64-bit → 96-bit 보안 강화 |
| NFC 저장 URL | `/w/{16자}` → URL 42바이트 | `/w/{24자}` → URL **54바이트** | 토큰 길이 변경 반영 |
| HMAC 표기 | `HMAC-SHA256(원본 + Salt)` (오기) | `HMAC-SHA256(key=SALT, message=원본)` | 암호학적 정확한 표기 |
| API Key 예시 | `sk_live_xxxx` (오해 소지) | `{발급 예정 API Key}` | 외부 문서 안전화 |
| `/w/{token}` 라우트 | 미구현(예정) | **v2.0 구현 완료** (2026-05-20) | 코드 반영 |
...
| NFC 저장값 | 평문 | **HMAC-SHA256 해시 토큰 24자리** |

### 1.2 개인정보보호법 검토 결과

- 생년월일 + 휴대폰 뒷 4자리 결합 = **개인정보보호법 제2조 제1호상 개인정보**
- 관련 판례: 대전지법 논산지원 2013고단17
- 대응 방식: **평문 노출 0건** → HMAC-SHA256 해시 토큰 방식으로 전환
...
| NFC/URL 저장값 | **HMAC-SHA256 해시 앞 24자리 hex 토큰** (평문 금지) |
| 전송 프로토콜 | HTTPS 필수 |
...
> **API 호출 방향**: 근로자가 현장에서 NFC 카드를 태그하면, 서원토건 SAFE-LINK 시스템이 하이정보 API를 호출하여 근로자 정보를 조회합니다. 하이정보 측에서 SAFE-LINK를 먼저 호출하는 구조가 아닙니다.
...
| 이름 | 형식 | 길이 | 설명 | 예시 |
|------|------|------|------|------|
| `token` | hex string | **24자** | HMAC-SHA256(key=SALT, message=원본) 앞 24자리 | `e7a3f9c2b1d4e85f12345678` |

### 3.3 Headers

```http
X-API-Key: {발급 예정 API Key}
...
```http
GET /safelink/v1/worker/e7a3f9c2b1d4e85f12345678 HTTP/1.1
...
해시:   64자리 hex string
                ↓
토큰:   앞 24자리 추출 → NFC/URL에 사용
        예: "e7a3f9c2b1d4e85f12345678"
```

> **주의**: HMAC은 단순 해시가 아니라 `key=SALT, message=원본` 구조입니다. Salt가 key 역할을 합니다.
...
def generate_worker_token(birthday: str, phone_last4: str) -> str:
    ...
    return digest[:24]  # 앞 24자리 (96-bit 보안)
```

### 5.3 구현 예시 (TypeScript — SAFE-LINK v2 적용)

```typescript
import { createHmac } from "crypto";

export function generateWorkerToken(birthday: string, phoneLast4: string): string {
  if (!/^\d{8}$/.test(birthday)) throw new Error("birthday must be YYYYMMDD");
  if (!/^\d{4}$/.test(phoneLast4)) throw new Error("phoneLast4 must be 4 digits");
...
URL 용량:     약 54바이트 → 여유 83바이트
라우트 상태:  SAFE-LINK v2.0 /w/[token] 페이지 구현 완료 (2026-05-20)
```
...
| 동일 토큰 충돌 | DB UNIQUE 제약 + 등록 시점 중복 검증. 24자(96-bit)로 충돌 확률 최소화 |
...
> **핵심 요약**: 평문 노출 0건 | 응답 필드 8개 | NFC 토큰 24자리 | DB AES-256 | `/w/{token}` 라우트 구현 완료
